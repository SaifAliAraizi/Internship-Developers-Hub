...
def setup_mfa():
    ...
    otp_uri = pyotp.totp.TOTP(secret).provisioning_uri(name=user, issuer_name="YourAppName")
...
def login():
    ...
    if request.method == "POST":
        ...
        # Retrieve MFA secret
        mfa_secret = rows[0].get("mfa_secret")
        if mfa_secret:
            # Require MFA token
            token = request.form.get("token")
            if not token:
                return apology("MFA token required", 403)

            # Verify MFA token
            totp = pyotp.TOTP(mfa_secret)
            if not totp.verify(token):
                return apology("Invalid MFA token", 403)
